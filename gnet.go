...
type Conn interface {
	...
	// 读取ring-buffer里入站的数据, 并不会移动read指针(类似于peek), 这些数据只有调用ResetBuffer方法才会被移除
	Read() (buf []byte)
	...
	// 给udp socket写数据, 允许在独立的协程中发送数据到udp socket
	...
	// 新开独立的协程去给客户端写数据(而不是在事件轮询的这个协程里)
...
type (
	...
	// 代表了服务的事件回调, 会返回一个action来管理连接的状态
	EventHandler interface {
		...
		// 当连接给server发送数据时触发, 通过c.read读取客户端数据, 通过action写入数据到客户端
