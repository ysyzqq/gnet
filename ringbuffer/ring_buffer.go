...
func (r *RingBuffer) Read(p []byte) (n int, err error) {
	...
	if r.w > r.r {
		...
		copy(p, r.buf[r.r:r.r+n]) // copy到p里
		r.r = r.r + n
		if r.r == r.w { // 读完了
			r.isEmpty = true
		}
...
func (r *RingBuffer) Write(p []byte) (n int, err error) {
	...
	if r.w >= r.r {
		c1 := r.size - r.w
		if c1 >= n { // 空间足够, 直接copy, w索引+n
			copy(r.buf[r.w:], p)
			r.w += n
		} else { // 空间不够, 先copy剩余c1的大小
			copy(r.buf[r.w:], p[:c1])
			c2 := n - c1
			copy(r.buf, p[c1:]) // 再从队头(0)处copy剩下的n-c1, w索引=c2
			r.w = c2
		}
	} else { // w小于r, 直接copy
		copy(r.buf[r.w:], p)
