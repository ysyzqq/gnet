...
// 唤醒阻塞的轮询器, 执行额外的异步任务队列
func (p *Poller) Trigger(job internal.Job) error {
	if p.asyncJobQueue.Push(job) == 1 { // 在第一个异步任务入队时标识, 轮询时通过这个去判断
		_, err := unix.Write(p.wfd, b) // 写入, 这时wfd变成可读状态, 会被epoll监听, 从而触发异步任务队列的执行
		return err
	}
	return nil
}
